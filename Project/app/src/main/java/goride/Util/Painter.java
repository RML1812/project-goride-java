...
public final class Painter
{
    ...
    public void drawPath(List<Tile> path, Grid model, Tile ROOT, Tile TARGET) 
    {
    ...
    public void highlightTile(Tile tile, long waitTime)
    {
        ...
        this.drawTile(tile, null, null, Tile.Type.KOSONG, waitTime);
    }
    

    public void walkPath(List<Tile> path, Grid model) 
    {
        this.executor.execute(
        () ->
        {
            path.stream().filter((tile) -> !(tile == model.getTarget())).map((tile) ->
            {
